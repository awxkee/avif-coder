...
template <class D, HWY_IF_F32_D(D)>
HWY_API VFromD<D> PromoteUpperTo(D d, Vec<Rebind<uint16_t, Twice<D>>> v) {
    const Rebind<uint32_t, decltype(d)> du32;
    const Rebind<uint16_t, decltype(d)> dh;
    return ConvertTo(d, PromoteTo(du32, UpperHalf(dh, v)));
}
